...
> *Status: v0 live on DigitalOcean · Owner: Julian · Last updated: 2026-06-14*
> *2026-06-14: **inkbytes.news marketing site (WordPress).** Public marketing site live at `inkbytes.news` — WordPress on the Hostinger box (`82.112.250.139`, `/docker/wordpress-yybr`, behind Traefik + LE cert), NOT the droplet. (1) Security-hardened (additive `docker-compose.override.yml` + mu-plugin): proxy-aware HTTPS, `DISALLOW_FILE_EDIT`, XML-RPC blocked at proxy, REST/`?author=` user-enumeration killed, HSTS+security headers, `/wp-login.php` rate-limited. (2) Homepage is now a **custom `inkbytes` theme** built 1:1 from `docs/marketing/index.html` (CTAs → reader at inkbytes.org). Theme source is now **committed to the monorepo** at `docs/marketing/inkbytes-theme/` (deploy/activate steps in its README). Deployed **manually** to Hostinger (not in CI). See memory `hostinger-wordpress-hardening`.*
