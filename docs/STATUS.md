...
> *2026-06-13: **Pre-enrich triage gate** (Curator ADR-0030) — a small local model (`qwen2.5:3b` on the Hostinger box) classifies each to-be-enriched article keep|junk from title+lede BEFORE the paid DeepSeek enrich, dropping filler (horoscopes/lottery/betting/deals/dead pages). Runs after the dedup fast-path, before enrich; fail-OPEN (timeout/error → keep); bounded to 2 concurrent. Shipped in **shadow mode** (logs `TRIAGE shadow-DROP` without dropping) to measure drop-rate + precision before enforcing. Local test (llama3.2:3b): clear junk dropped, real news + betting-*investigation* kept. Expected ~15-25% fewer enrich calls (~$2-4/day) once enforced.*
