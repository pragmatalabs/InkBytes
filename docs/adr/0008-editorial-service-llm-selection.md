...
>
> **Outlook v1.1 (2026-07-03, committed, NOT deployed):** (1) **date-grouped timeline** — the `/outlook` index now groups editions by `edition_date` (newest first, a "Latest" badge on the top day) via a new `GET /outlook/archive?lang=&days=` endpoint (guarded); cards deep-link to `/outlook/{topic}?date=`. (2) **Share** — `outlook-actions.tsx` gains a Share button (Web Share API → copy-link fallback) beside Copy/Export/Print. (3) **Discoverability** — an Outlook promo banner on the home feed + an **Outlook item in the mobile bottom nav** (`OutlookIcon` sunrise glyph); desktop header already had it. (4) **Bug fix (was live-broken):** `GET /outlook?...&date=` 500'd — asyncpg needs a `date` object, not the raw string, for the `edition_date` param (`DataError: 'str' has no attribute 'toordinal'`). The deployed endpoint's `date` path (Past-editions picker + the new grouped cards) was broken; now parses `date.fromisoformat` with a graceful malformed→latest fallback. Caught by local testing (seeded 2 dates, booted curator API + reader). tsc + AST clean.
>
