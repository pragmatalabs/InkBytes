...
class StorageService:
    ...
    def save_scraping_session(self, scraping_session):
        ...
        try:
            # Extract filename from session if available
            if isinstance(scraping_session, dict) and "data" in scraping_session:
                if "results_staging_file_name" in scraping_session["data"]:
                    ...
                    file_path = os.path.join(self.config.storage.staging.local.scraping(), file_name)
    ...
    def _load_articles_from_staging_file(self, file_path: str) -> List[Dict[str, Any]]:
        """Load article payloads from the TinyDB staging file format."""
            ...
            if result:
                ...
                if self.message_service and self.message_service.connection and self.message_service.connection.is_open:
                    ...
                    try:
                        ...
                        self.logger.error(f"Error parsing JSON from file: {local_file_path}")
                    except Exception as e:
                        self.logger.error(f"Error publishing event for file {file_name}: {e}")
    ...
    def execute_files_move(self):
        ...
        try:
            ...
            if result:
                ...
                # If message_service is available, publish events for each file
                if self.message_service and self.message_service.connection and self.message_service.connection.is_open:
                    # Get list of uploaded files to publish events
                    files = []
                    for filename in os.listdir(local_path):
                        if filename.endswith('.db.json'):
                            files.append(os.path.join(local_path, filename))
                    
                    for file_path in files:
                        filename = os.path.basename(file_path)
                        s3_path = f"{folder}/{filename}"
                        
                        # Read session info from the file to get metadata
                        try:
                            ...
                            self.logger.info(f"Published event for file: {filename}")
                        except json.JSONDecodeError:
                            self.logger.error(f"Error parsing JSON from file: {file_path}")
                        except Exception as e:
                            self.logger.error(f"Error publishing event for file {filename}: {e}")
            else:
