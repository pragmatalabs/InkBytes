...
| LLM resilience | Generic provider **failover**: active provider hits a credit/quota wall (e.g. DeepSeek balance $0 → 402) → same call transparently retried on `LLM_FALLBACK_PROVIDER` (prod = `openrouter`, a different balance) instead of halting the worker. Env-only (never in `curator_settings`). Plus a single-instance (`--api-only`) **DeepSeek balance monitor** + a best-effort **ops webhook** (`OPS_ALERT_WEBHOOK_URL`, Slack/Telegram) that pages *before* (low balance) and *at* (both providers dead) a freeze. Inert with no env set. Auto-recharge is a DeepSeek-account setting, NOT in Curator. Curator ADR-0044 (fixes the 2026-08-01 14h freeze). |
| Editorial service (design) | Daily per-theme editorial persona — **separate service, NOT a Curator skill** (ADR-0008, 2026-06-11). Provider-pluggable LLM (`ollama \| deepseek \| anthropic`). Bake-off: **gemma4 12B = Spanish quality floor**, 4B-class fails. Dev = local gemma4 (Mac); prod = DeepSeek until the 16 GB droplet upgrade, then local. Themes from `articles.theme` (15 verticals as of Curator ADR-0032; was 8) — `events.topic` is empty, don't use it. |
...
- **Chromium in Docker without an init/zombie-reaper (`init: true`) + `pids_limit`**. IllustrateSkill launches headless Chromium per event. When a launch crashes, Chromium's helper processes get reparented to PID 1; the bare `python main.py` is **not a subreaper**, so they accumulate as zombies. Over days this exhausted the container's PID table (~9.3k chrome procs, `pids.current≈pids.max`) → `pthread_create: Resource temporarily unavailable (EAGAIN)` → every new launch `SIGTRAP`s → **no video rail**. ⚠️ The SIGTRAP **looks like the seccomp issue above but isn't** — if Chromium suddenly stops launching, check `pids.current` vs `pids.max` and the chrome-proc count BEFORE chasing seccomp/flags/versions. Recovery: restart the worker(s) (reaps the leak, `pids.current`→single digits) — the worker now runs as **`deploy.replicas`** (2026-07-30), so it has no fixed container name: use `docker compose -f infra/docker-compose.prod.yml -f infra/docker-compose.do.yml restart inkbytes-curator-worker` (service → all replicas) or target one by its `inkbytes-inkbytes-curator-worker-<n>` name. Durable fix: `init: true` (tini reaps zombies) + `pids_limit: 4096` (caps blast radius — it was unbounded, so the leak ate the whole shared droplet's process table) on the worker in `docker-compose.do.yml` (2026-06-25, extends ADR-0011).
...
- **Adding a Curator env var to `infra/.env` but NOT to the compose `environment:` block**. The `inkbytes-curator-api` / `inkbytes-curator-worker` services in `infra/docker-compose.prod.yml` use **explicit passthrough** (`VAR: ${VAR:-default}`), not `env_file:`. A var present only in `infra/.env` **never reaches the container** — it must ALSO be listed in *both* the api and worker `environment:` blocks. This silently left ADR-0044 failover + Telegram alerting inert (`OpsAlerter().enabled == False`, no `LLM_FALLBACK_PROVIDER` in the worker) even though `.env` was correct — an hour lost on 2026-08-01. After adding an env var to a curator service, recreate with `docker compose -f infra/docker-compose.prod.yml -f infra/docker-compose.do.yml --env-file infra/.env up -d inkbytes-curator-api inkbytes-curator-worker` (no rebuild needed for an env-only change). Verify it landed: `docker exec inkbytes-curator-api printenv <VAR>`. (Curator ADR-0044.)
- **Writing `pages.freshness_at` ONLY at synthesis**. `freshness_at` is a *derived* column (`max(scraped_at)` over the event's articles) but `SynthesizeSkill` was its only writer. Re-synthesis of a published event is throttled (ADR-0035 watermark), so a fresh article attaching to an already-published event bumps the material clock (`events.last_material_update_at`, ADR-0033) yet leaves `freshness_at` **frozen at first-synthesis time**. The lifecycle feed (`GET /events`) orders by the material clock but *displays* `freshness_at` → evolving events re-float to the top showing hours/days-stale ages (a healthy pipeline reads as "no news update in 2h"; on 2026-08-01, 62% of the top 50 showed >2h old while their newest article was scraped minutes earlier). `ClusterSkill._touch_page_freshness()` now recomputes `freshness_at = max(scraped_at)` on every **material** attach (both attach paths), gated on `material` so a tangential re-mention refreshes neither ranking nor display (Curator ADR-0043). If you add another writer that advances the material clock, it must refresh `freshness_at` too.
...
- **Rendering an `<a>`/`<Link>` inside a Reader card `<Link>`**. Feed/entity cards are anchors; a child that renders its own anchor (an "also es/en" chip, a CC-attribution overlay, any inline link) produces nested `<a>` — invalid HTML. The parser DOM-corrects it (closes the outer anchor early → broken card layout) and React throws a hydration error → the whole feed re-renders client-side for every visitor. And the bug can be **data-dependent** (EventCover's credit only renders when a BY/BY-SA cover lands in a card slot), so a live DOM spot-check passing proves nothing — grep for the class, audit every `<a>` render's call sites. Interactive children of link-cards must be `<button>`s (`router.push` internal / `window.open` external) with `preventDefault`+`stopPropagation` (ADR-R-0010, 2026-07-12).
