...
> *Status: **P0 + P1a + P1b + Unsplash + entity-disambiguation implemented** (2026-06-28, committed) · Owner: Julian De La Rosa · Date: 2026-06-25*
>
> **Entity disambiguation (2026-06-28):** the Wikimedia path picked the wrong real-world entity for ambiguous names — "Proton" (the privacy company / Lumo) resolved to **Proton Holdings, the Malaysian carmaker** (Wikidata's top bare-string hit) → a car-dealership photo on an AI-chatbot story. Fix: (1) `wikimedia_entity_cover(entity, client, context)` now fetches the top-7 `wbsearchentities` candidates and re-ranks by **story-context overlap** (headline + theme + co-entities) — "Proton" + tech context → Proton AG (which has no P18 → falls through, no wrong image). Zero extra API calls (limit 7 vs 1 + scoring). (2) the Openverse/Unsplash keyword fallback now uses **LOC-or-theme, never an ambiguous ORG brand name** (orgs are served by the Wikimedia canonical step; else the theme concept). Context threaded from `_cover_safe` (`get_event_cover_inputs` returns headline + entity_terms) and `backfill_covers.py`. **Deploy + `backfill_covers.py --overwrite` re-fixes existing wrong-entity covers.** Validated: Proton → "data center servers" generic tech image; Venezuela → unchanged canonical.
>
