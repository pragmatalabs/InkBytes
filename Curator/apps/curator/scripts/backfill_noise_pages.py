#!/usr/bin/env python
"""One-time backfill: un-publish existing horoscope/lottery/betting pages (Curator ADR-0021).

The non-news filler gate only runs at synthesis, so pages published before it
shipped keep their filler covers in the feed. This re-applies the same rule to
every published page and un-publishes the ones that are filler:

  • headline is filler (is_excluded), OR
  • a strict majority of the event's article titles are filler.
...
Run inside the Curator container:
  docker exec inkbytes-curator-worker python scripts/backfill_noise_pages.py
  docker exec inkbytes-curator-worker python scripts/backfill_noise_pages.py --apply
"""
...
def _page_is_noise(headline: str, titles: list[str]) -> str | None:
    """Mirror of the synthesize gate. Returns a reason label, or None if clean."""
    r = exclusion_reason(headline)
    if r:
        return f"headline:{r}"
    clean_titles = [t for t in titles if t]
    if clean_titles:
        noise = sum(1 for t in clean_titles if is_excluded(t))
        if noise * 2 > len(clean_titles):
            return f"cluster:{noise}/{len(clean_titles)}"
    return None
...
async def _run(config_path: str, apply: bool) -> int:
    ...
    try:
        ...
        for r in rows:
            reason = _page_is_noise(r["headline"], list(r["titles"]))
            if reason:
                flagged.append((r["id"], reason, r["headline"]))

        print(f"[backfill] {len(rows)} published pages · {len(flagged)} filler "
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Un-publish horoscope/lottery/betting pages (ADR-0021)")
