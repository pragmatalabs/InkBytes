...
def build_app(app: Application) -> FastAPI:
    ...
    async def get_outlook(response: Response, theme: str, lang: str = "es",
        ...
        async with app.db.pool.acquire() as conn:  # type: ignore[union-attr]
            if not await conn.fetchval("SELECT to_regclass('public.editorials')"):
                return {}
            # audio_url / cover_url are added by the Editorial migrations 002/003
            # (ADR-0011/0012). Guard so a Curator deploy landing before them can't 500.
            colrows = await conn.fetch(
                "SELECT column_name FROM information_schema.columns "
                "WHERE table_schema='public' AND table_name='editorials'")
            colnames = {r["column_name"] for r in colrows}
            audio_sel = ", audio_url" if "audio_url" in colnames else ", NULL AS audio_url"
            cover_sel = ", cover_url" if "cover_url" in colnames else ", NULL AS cover_url"
            ed = await conn.fetchrow(
                "SELECT theme, language, edition_date, persona, headline, body_md, "
                "       event_ids, model" + audio_sel + cover_sel + " "
        ...
        return {
            ...
            "audio_url": ed["audio_url"], "cover_url": ed["cover_url"],
    ...
    async def get_event(event_id: str) -> dict[str, Any]:
        ...
        if not row:
            # Merged away (ADR-0040)? Return the survivor id so the Reader can
            # 302-redirect the old URL instead of 404-ing.
            survivor = await app.db.fetch_merged_into(event_id)
            if survivor:
                return {"event_id": event_id, "merged_into": survivor}
    ...
    async def get_entity(name: str, response: Response) -> dict[str, Any]:
        ...
        graph just to show one story's entity. `name` is the lowercased entity
        key (name_key). 404 when the entity isn't in any published event.
        """
        ...
        async with app.db.pool.acquire() as conn:  # type: ignore[union-attr]
            row = await conn.fetchrow(
                """
                WITH published AS (
                    SELECT e.id AS event_id, p.id AS page_id, p.headline,
                           p.freshness_at, e.source_count
                      FROM events e JOIN pages p ON p.event_id = e.id
                     WHERE p.published_at IS NOT NULL AND e.status = 'published'
                ),
                ev_ents AS (
                    SELECT DISTINCT ON (a.event_id, LOWER(ent.name))
                           a.event_id, pub.page_id, pub.headline, pub.freshness_at,
                           pub.source_count, LOWER(ent.name) AS name_key, ent.name AS name_orig
                      FROM articles a
                      JOIN published pub ON pub.event_id = a.event_id
                      JOIN entities  ent ON ent.article_id = a.id
                     WHERE LENGTH(ent.name) >= 2
                ...
                target AS (SELECT * FROM ev_ents WHERE name_key = $1),
                conns AS (
                    SELECT o.name_key AS id, MIN(o.name_orig) AS label,
                           COUNT(DISTINCT o.event_id) AS weight
                      FROM target t
                      JOIN ev_ents o ON o.event_id = t.event_id AND o.name_key <> t.name_key
                     GROUP BY o.name_key
                ),
                recent AS (
                    SELECT DISTINCT ON (page_id) page_id AS id, headline,
                           source_count, freshness_at
                      FROM target
                     ORDER BY page_id, freshness_at DESC
                ),
                dtype AS (
                ...
                SELECT
                    (SELECT name_orig FROM target ORDER BY page_id LIMIT 1)            AS label,
                    (SELECT type FROM dtype)                                           AS type,
                    (SELECT COUNT(DISTINCT event_id) FROM target)                      AS event_count,
                    (SELECT COUNT(DISTINCT event_id) FROM target
                       WHERE freshness_at > NOW() - INTERVAL '24 hours')               AS today_count,
                    (SELECT COUNT(*) FROM conns)                                       AS connection_count,
                    COALESCE((SELECT JSON_AGG(x) FROM
                        (SELECT id, headline, source_count, freshness_at
                           FROM recent ORDER BY freshness_at DESC LIMIT 15) x), '[]'::json) AS recent_events,
